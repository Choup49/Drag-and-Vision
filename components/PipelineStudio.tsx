...
import { PipelineNode, PipelineConnection, NodeType, NodeDefinition, Position, Language, Challenge, ApiConfig, DroidCamConfig, OnnxConfig, LogicConfig } from '../types';
import { 
    Plus, Trash2, Download, X, Zap, Move, FileVideo, Camera, Cpu, Box, Layers, 
    ZoomIn, ZoomOut, RotateCcw, Info, Settings2, CheckCircle, AlertCircle, 
    Play, Map as MapIcon, Globe, DownloadCloud, Eye, Network, Sparkles, 
    Brain, Gauge, ImageIcon, Wifi, UploadCloud, Loader2, Binary, Server 
} from 'lucide-react';
...
const PipelineStudio: React.FC<PipelineStudioProps> = ({ customNodes, language, activeChallenge, onExitChallenge, onImportCommunityNode, onChallengeComplete }) => {
  ...
  const generatePython = () => {
    ...
    if (executionOrder.length === 0) { setGeneratedCode("# Error: No Source Node found."); setShowCode(true); return; }
    ...
    const mainSource = nodes.find(n => n.uuid === executionOrder[0])!;
  ...
  const generateFlask = () => {
    ...
    nodes.forEach(n => getNodeDef(n.defId)?.requiredImports?.forEach(imp => imports.add(imp)));
    imports.forEach(imp => {
        if (!['cv2', 'numpy'].includes(imp)) code += `import ${imp}\n`;
    });

    code += `\n# --- PIPELINE SETUP ---\npipeline_data = {}\n`;

    const executionOrder = getExecutionOrder();
    if (executionOrder.length === 0) { setGeneratedCode("# Error: No nodes found."); setShowCode(true); return; }

    executionOrder.forEach(uuid => {
        const node = nodes.find(n => n.uuid === uuid)!;
        const def = getNodeDef(node.defId);
        if (!def || def.type === NodeType.SOURCE) return; 
    ...
    code += `\n@app.route('/process', methods=['POST'])\ndef process_pipeline():\n    global pipeline_data\n    data = request.json\n    if 'image' not in data: return jsonify({'error': 'No image provided'}), 400\n    \n    # Decode Image\n    img_data = base64.b64decode(data['image'])\n    nparr = np.frombuffer(img_data, np.uint8)\n    input_frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR)\n    \n    # Use input_frame as starting point\n    last_output = input_frame\n`;
    
    executionOrder.forEach(uuid => {
        const node = nodes.find(n => n.uuid === uuid)!;
        const def = getNodeDef(node.defId);
        if (!def || def.type === NodeType.SOURCE || def.type === NodeType.OUTPUT) return; 

        const nodeIdShort = uuid.split('-')[0];
        const inputVar = "last_output";
        const outputVar = `frame_${nodeIdShort}`;
    ...
    <div className="flex h-full bg-slate-950 overflow-hidden">
      ...
      <div className={`flex-1 relative bg-[#0b1121] overflow-hidden ${isPanning ? 'cursor-grabbing' : 'cursor-grab'}`} ref={containerRef} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseDown={handleCanvasMouseDown} onWheel={handleWheel}>
        ...
        <div className="absolute top-4 right-4 flex gap-3 z-30">
             ...
             <button onClick={generateFlask} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg shadow-lg shadow-indigo-500/20 font-medium border border-indigo-400/30"><Server className="w-4 h-4" /> {t.exportFlask}</button>
             <button onClick={generatePython} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg shadow-lg shadow-cyan-500/20 font-medium border border-cyan-400/30"><Download className="w-4 h-4" /> {t.export}</button>
        </div>
